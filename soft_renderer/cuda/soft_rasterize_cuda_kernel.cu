...
template <typename scalar_t>
__device__ __forceinline__ scalar_t forward_sample_texture(const scalar_t *texture, const scalar_t *w, const int R, const int k, const int channel_size, const int texture_sample_type) {
    scalar_t texture_k;
    ...
    if (texture_sample_type == 1) { // sample vertex color
        texture_k = w[0] * texture[k] + w[1] * texture[channel_size+k] + w[2] * texture[channel_size*2+k];
    }
    return texture_k;
}
...
__global__ void forward_soft_rasterize_cuda_kernel(
    ...
    const scalar_t *texture = &textures[bn * nf * texture_size * channel_size] - texture_size * channel_size;
    ...
    // scalar_t soft_color[channel_size + 1];
    // int max_dim = 100;
    scalar_t soft_color[9 + 1];
    for (int k=0; k<channel_size; k++){
        soft_color[k] = 1.;
    }
    soft_color[channel_size] = 0.;
    
    if (func_id_alpha == 2) soft_color[channel_size] = 1.;
    scalar_t softmax_sum = exp(eps / gamma_val);
    scalar_t softmax_max = eps;
    for (int k = 0; k < channel_size; k++) {
        if (func_id_rgb == 0) { // hard assign, set to background
            soft_color[k] = soft_colors[(bn * (channel_size+1) + k) * (is * is) + pn];
        } else
        if (func_id_rgb == 1) {
            soft_color[k] = soft_colors[(bn * (channel_size+1) + k) * (is * is) + pn] * softmax_sum; // initialize background color
        }
    }
    ...
    for (int fn = 0; fn < nf; fn++) {
        face += 9;
        texture += texture_size * channel_size;
        face_info += 27;
        ...
        if (func_id_alpha == 0) { // hard assign
            if (soft_fragment > 0.5) soft_color[channel_size] = 1.;
        } else
        if (func_id_alpha == 1) { // Sum
            soft_color[channel_size] += soft_fragment;
        } else 
        if (func_id_alpha == 2) { // Logical-Or
            soft_color[channel_size] *= 1. - soft_fragment;
        }
        ...
        if (func_id_rgb == 0) { // Hard assign
            if (zp < depth_min && check_pixel_inside(w) && (double_side || check_face_frontside(face))) {
                ...
                face_index_min = fn;
                for (int k = 0; k < channel_size; k++) {
                    soft_color[k] = forward_sample_texture(texture, w_clip, texture_res, k, channel_size, texture_sample_type);
                }
            }
        } else
        if (func_id_rgb == 1) { // D * Softmax (Z)
            if (check_face_frontside(face) || double_side) {
                ...
                for (int k = 0; k < channel_size; k++) {
                    const scalar_t color_k = forward_sample_texture(texture, w_clip, texture_res, k, channel_size, texture_sample_type);
    ...
    if (func_id_alpha == 0) {
        soft_colors[(bn * (channel_size+1) + channel_size) * (is * is) + pn] =  soft_color[channel_size];
    } else
    if (func_id_alpha == 1) {
        soft_colors[(bn * (channel_size+1) + channel_size) * (is * is) + pn] =  soft_color[channel_size] / nf;
    } else 
    if (func_id_alpha == 2) {
        soft_colors[(bn * (channel_size+1) + channel_size) * (is * is) + pn] =  1. - soft_color[channel_size];
    }

    if (func_id_rgb == 0) {
        if (face_index_min != -1)
            for (int k = 0; k < channel_size; k++) {
                soft_colors[(bn * (channel_size+1) + k) * (is * is) + pn] = soft_color[k];
            }
    ...
    if (func_id_rgb == 1) {
        for (int k = 0; k < channel_size; k++) {
            soft_colors[(bn * (channel_size+1) + k) * (is * is) + pn] = soft_color[k] / softmax_sum;
        }
...
__global__ void backward_soft_rasterize_cuda_kernel(
    ...
    for (int fn = 0; fn < nf; fn++) {
        ...
        scalar_t* grad_texture = &grad_textures[(bn * nf + fn) * texture_size * channel_size];
        ...
        // aggragate for alpha channel
        scalar_t C_grad_xy_alpha = grad_soft_colors[(bn * (channel_size+1) + channel_size) * (is * is) + pn];
        if (func_id_alpha == 0) { // hard assign
        ...
        if (func_id_alpha == 2) { // Logical-Or
            C_grad_xy_alpha *= (1 - soft_colors[(bn * (channel_size+1) + channel_size) * (is * is) + pn]) / max(1 - soft_fragment, 1e-6);
        }
            ...
            if (fn == softmax_max) {
                for (int k = 0; k < channel_size; k++) {
                    for (int j = 0; j < texture_size; j++) {
                        atomicAdd(&grad_texture[channel_size * j + k], backward_sample_texture(grad_soft_colors[(bn * (channel_size) + k) * (is * is) + pn], w, texture_res, j, texture_sample_type));
                    }
                }
            }
        } else
        if (func_id_rgb == 1 && (check_face_frontside(face) || double_side)) { // Softmax (Z * D)
            ...
            for (int k = 0; k < channel_size; k++) {
                const scalar_t grad_soft_color_k = grad_soft_colors[(bn * (channel_size+1) + k) * (is * is) + pn];

                for (int j = 0; j < texture_size; j++) {
                    const scalar_t grad_t = backward_sample_texture(grad_soft_color_k, w, texture_res, j, texture_sample_type);
                    atomicAdd(&grad_texture[(channel_size+1) * j + k], zp_softmax * grad_t);
                }

                const scalar_t color_k = forward_sample_texture(texture, w, texture_res, k, channel_size, texture_sample_type);
                C_grad_xyz_rgb += grad_soft_color_k * (color_k - soft_colors[(bn * (channel_size+1) + k) * (is * is) + pn]);
            }
...
std::vector<at::Tensor> forward_soft_rasterize_cuda(
    ...
    const auto channel_size = textures.size(3);
    // print(channel_size)
