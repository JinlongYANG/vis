...
def projection(vertices, K,R,t, dist_coeffs, orig_size, eps=1e-9):
    ...
    # print(P['K'])
    # K = torch.from_numpy(P['K'].astype('float32')).cuda()
    # R = torch.from_numpy(P['R'].astype('float32')).cuda()
    # t = torch.from_numpy(P['t'].astype('float32')).cuda()
    ...
    # print(torch.max(vertices[:,:,2]))
    # print(torch.min(vertices[:,:,2]))
