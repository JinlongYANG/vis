...
class Projection(nn.Module):
    def __init__(self, K, R, t, dist_coeffs=None, orig_size=512):
        ...
        if isinstance(self.K, np.ndarray):
            self.K = torch.from_numpy(self.K.astype('float32')).cuda()
        if isinstance(self.R, np.ndarray):
            self.R = torch.from_numpy(self.R.astype('float32')).cuda()
        if isinstance(self.t, np.ndarray):
            self.t = torch.from_numpy(self.t.astype('float32')).cuda()
        # if isinstance(self.P, np.ndarray):
        #     self.P = torch.from_numpy(self.P).cuda()
        # if self.P is None or P.ndimension() != 3 or self.P.shape[1] != 3 or self.P.shape[2] != 4:
        #     raise ValueError('You need to provide a valid (batch_size)x3x4 projection matrix')
    ...
    def forward(self, vertices):
        vertices = srf.projection(vertices, self.K, self.R, self.t, self.dist_coeffs, self.orig_size)
        return vertices
...
class Transform(nn.Module):
    ...
    def forward(self, mesh, isVertices=False):
        if isVertices is not True:
            mesh.vertices = self.transformer(mesh.vertices)
        else:
